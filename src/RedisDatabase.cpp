...
void RedisDatabase::purgeExpired(){
    std::lock_guard<std::mutex>lock(db_mutex);
    auto now=std::chrono::steady_clock::now();//check for time
...
ssize_t RedisDatabase::llen(const std::string&key){//check number of items and return t
    std::lock_guard<std::mutex>lock(db_mutex);
    auto it=list_store.find(key);
    if(it!=list_store.end())
...
int RedisDatabase::lrem(const std::string& key,int count ,std::string& value){
        ...
        if(count==0){
            ...
            //Remove the element from head to tail
...
/*
key-value (k)
kv_store["name"]="Himanshu"
kv_store["city"]="Dehradun"

list (L)
list_store["fruits"]=["apple","banana","orange"]
lsit_store["colours"]=["red","green","blue"]

Hash (H)
hash_store["user:100"]={
    {"name","Bob"},
    {"age","30"},
    {"mail","example@gmial.com"}
};
*/
bool RedisDatabase::load(const std::string& filename){
    ...
    while(std::getline(ifs,line)){
        ...
        if(type=='K'){
            ...
            while(iss>>pair){
                auto pos=pair.find(':');
                if(pos!=std::string::npos){ //left part will be the key and right part will be the value
